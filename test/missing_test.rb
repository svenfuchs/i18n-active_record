...
class I18nActiveRecordMissingTest < I18n::TestCase
  ...
  [false, true].each do |cache_translations|
    ...
    test "can persist interpolations, Cache Translations: #{cache_translations}" do
    ...
    test "lookup persists the key, Cache Translations: #{cache_translations}" do
      I18n.t('foo.bar.baz')
    ...
    test "lookup does not persist the key twice, Cache Translations: #{cache_translations}" do
      2.times { I18n.t('foo.bar.baz') }
      assert_equal 1, I18n::Backend::ActiveRecord::Translation.count
      assert I18n::Backend::ActiveRecord::Translation.locale(:en).find_by_key('foo.bar.baz')
    end

    test "lookup persists interpolation keys when looked up directly, Cache Translations: #{cache_translations}" do
    ...
    test "creates one stub per pluralization, Cache Translations: #{cache_translations}" do
    ...
    test "creates no stub for base key in pluralization, Cache Translations: #{cache_translations}" do
      I18n.t('foo', :count => 999)
    ...
    test "creates a stub when a custom separator is used, Cache Translations: #{cache_translations}" do
    ...
    test "creates a stub per pluralization when a custom separator is used, Cache Translations: #{cache_translations}" do
    ...
    test "creates a stub when a custom separator is used and the key contains the flatten separator (a dot character), Cache Translations: #{cache_translations}" do
      key = 'foo|baz.zab'
