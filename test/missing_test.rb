...
class I18nActiveRecordMissingTest < I18n::TestCase
  ...
  def setup
    ...
    store_translations(:en, bar: 'Bar', i18n: { plural: { keys: [:zero, :one, :other] } })
  end
  ...
  class WithoutCacheTest < I18nActiveRecordMissingTest
    ...
    test "creates a stub when a custom separator is used" do
      ...
      update_attributes(I18n::Backend::ActiveRecord::Translation.locale(:en).lookup("foo.baz").first, value: 'baz!')
    ...
    test "creates a stub when a custom separator is used and the key contains the flatten separator (a dot character)" do
      ...
      update_attributes(I18n::Backend::ActiveRecord::Translation.locale(:en).lookup("foo.baz\001zab").first, value: 'baz!')
