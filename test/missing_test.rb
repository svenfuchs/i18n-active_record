...
class I18nActiveRecordMissingTest < I18n::TestCase
  ...
  [false, true].each do |cache_translations|
    ...
    test "can persist interpolations, #{cache_suffix}" do
    ...
    test "lookup persists the key, #{cache_suffix}" do
      I18n.t('foo.bar.baz')
    ...
    test "lookup does not persist the key twice, #{cache_suffix}" do
      2.times { I18n.t('foo.bar.baz') }
      assert_equal 1, I18n::Backend::ActiveRecord::Translation.count
      assert I18n::Backend::ActiveRecord::Translation.locale(:en).find_by_key('foo.bar.baz')
    end

    test "lookup persists interpolation keys when looked up directly, #{cache_suffix}" do
    ...
    test "creates one stub per pluralization, #{cache_suffix}" do
    ...
    test "creates no stub for base key in pluralization, #{cache_suffix}" do
      I18n.t('foo', :count => 999)
    ...
    test "creates a stub when a custom separator is used, #{cache_suffix}" do
    ...
    test "creates a stub per pluralization when a custom separator is used, #{cache_suffix}" do
    ...
    test "creates a stub when a custom separator is used and the key contains the flatten separator (a dot character), #{cache_suffix}" do
      key = 'foo|baz.zab'
