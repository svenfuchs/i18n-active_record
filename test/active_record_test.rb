...
class I18nBackendActiveRecordTest < I18n::TestCase
  ...
  [false, true].each do |cache_translations|
    ...
    test "store_translations does not allow ambiguous keys (2), Cache Translations: #{cache_translations}" do
      I18n::Backend::ActiveRecord::Translation.delete_all
      I18n.backend.store_translations(:en, :foo => { :bar => 'bar' })
      I18n.backend.store_translations(:en, :foo => { :baz => 'baz' })
      I18n.backend.store_translations(:en, :foo => 'foo')
    ...
    test "can store translations with keys that are translations containing special chars, Cache Translations: #{cache_translations}" do
      I18n.backend.store_translations(:es, :"Pagina's" => "Pagina's" )
      assert_equal "Pagina's", I18n.t(:"Pagina's", :locale => :es)
    end

    test "missing translations table does not cause an error in #available_locales, Cache Translations: #{cache_translations}" do
    ...
    test "expand_keys, Cache Translations: #{cache_translations}" do
      assert_equal %w(foo foo.bar foo.bar.baz), I18n.backend.send(:expand_keys, :'foo.bar.baz')
    end

    test "available_locales returns uniq locales, Cache Translations: #{cache_translations}" do
      I18n::Backend::ActiveRecord::Translation.delete_all
      I18n.backend.store_translations(:en, :foo => { :bar => 'bar' })
      I18n.backend.store_translations(:en, :foo => { :baz => 'baz' })
    ...
    test "the default configuration has cleanup_with_destroy == false, Cache Translations: #{cache_translations}" do
      refute I18n::Backend::ActiveRecord.config.cleanup_with_destroy
    end

    test "the configuration supports cleanup_with_destroy being set, Cache Translations: #{cache_translations}" do
    ...
    test "fetching subtree of translations, Cache Translations: #{cache_translations}" do
      I18n::Backend::ActiveRecord::Translation.delete_all
    ...
    test "build_translation_hash_by_key, Cache Translations: #{cache_translations}" do
    ...
    test "returning all keys via ., Cache Translations: #{cache_translations}" do
      expected_hash = {:foo => { :bar => 'bar', :baz => 'baz' }}
      assert_equal expected_hash, I18n.t('.')
    end

    test "accessing keys with a trailing/leading period, Cache Translations: #{cache_translations}" do
    ...
    test "returning all keys via . when there are no keys, Cache Translations: #{cache_translations}" do
    ...
    test "intially unitinitialized, Cache Translations: #{cache_translations}" do
    ...
    test "translations returns all translations, Cache Translations: #{cache_translations}" do
    ...
    test "translations initialized with do_init argument, Cache Translations: #{cache_translations}" do
      expected_hash = { :en => { :foo => { :bar => 'bar', :baz => 'baz' } } }
      refute I18n.backend.initialized?
